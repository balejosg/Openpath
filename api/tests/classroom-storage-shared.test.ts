...
await test('classroom-storage shared helpers', async (t) => {
  await t.test('normalize machine and policy values', () => {
    const machineKey = buildMachineKey('classroom-1', ' Lab-PC-01 ');
    assert.ok(machineKey.includes('--'));

    assert.equal(
      machineHostnameMatches(
        { hostname: 'lab-pc-01', reportedHostname: 'Lab-PC-01 ' },
        'lab-pc-01'
      ),
      true
    );
  ...
  await t.test('accepts Windows-normalized scoped machine keys', () => {
